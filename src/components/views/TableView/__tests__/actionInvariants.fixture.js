...
export const fixtures = [
  ...
  },
  {
    ...
    inputs: {
      ...
      actionSequence: [], // straddle has no action representation
      absentSeats: [],
      dealerButtonSeat: 8,
      bigBlindSeat: 1,
      preflop_context: 'open',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'Straddle DOES NOT EXIST in code. PRIMITIVE_ACTIONS has no straddle constant. Pot calculation, action order, and BB option logic all assume 2 blinds. Owner scope decided 2026-05-02 (WS-002 / SPR-010): UTG + BTN only, UTG > BTN precedence, no re-straddle. See STRADDLE COVERAGE section below for the full row catalog the fix wave must absorb.',
  },
  {
    id: 'INV-S-008',
    ...
    expected_per_spec: [CALL, RAISE, FOLD], // BB facing straddle = facing a $2 raise; CHECK NOT legal
    actual_today: [CHECK, RAISE, FOLD], // isBBOption fires because no RAISE in seq — but straddle SHOULD count as a raise
    status: 'spec_gap',
    bug_id: null,
    comment: 'CRITICAL straddle gap: isBBOption transform fires (no preflop "raise" recorded), so BB sees CHECK. But facing a $2 straddle, CHECK is illegal. Cannot pin as pinned_bug because straddle is not representable in actionSequence today — there is no entry to put a "raise" on. Owner scope decided 2026-05-02 (WS-002 / SPR-010): UTG + BTN only, UTG > BTN precedence, no re-straddle. INV-S-015 below pins the symmetric BTN-straddle case for the same isBBOption gap.',
  },
  {
    id: 'INV-S-009',
  ...
  // ===========================================================================
  {
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'BTN facing UTG straddle, all middle seats folded. Same shape as INV-S-007 (UTG+1 first to act) — once everyone folds, BTN inherits the open option. Pinned to confirm position-independence on the open spot. Inputs cannot record the UTG straddle itself; assume UTG=seat 3 has posted (special_state="straddle" is the only carrier).',
  },
  {
    id: 'INV-S-011',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'SB facing UTG straddle, action folded around. SB completes by CALLING (not checking) — facing $2 straddle is the same shape as facing any open. CHECK must NOT render. The SB-completing transform (WS-129) fires today on no-raise and would coincidentally produce the wrong [CHECK, RAISE, FOLD] when smallBlindSeat is wired into inputs; the spec_gap classification holds because the input — a posted UTG straddle — is not representable.',
  },
  {
    id: 'INV-S-012',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'UTG straddler last-action option: SB and BB called the straddle, action returns to UTG straddler. Analogous to BB option but on a 2× BB pot where the straddler\'s own posted blind is the effective last "raise". CommandStrip\'s isBBOption check would NOT fire (hero seat ≠ BB); the straddler-option transform path is unimplemented entirely — production must add a parallel isStraddlerOption guard.',
  },
  {
    id: 'INV-S-013',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'UTG straddler facing a raise after their posted blind. Standard facing-action button shape (CALL/RAISE/FOLD); the special_state aspect is action-order — straddler acts LAST regardless of seat-order normality. Production fix: getNextSeat must route last preflop action to straddler when special_state="straddle". The button shape itself is correct because hasBetOrRaiseOnStreet detects the seat-5 raise.',
  },
  {
    id: 'INV-S-014',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'BTN straddle, UTG (hero seat 3) opens — first to act preflop. Action proceeds clockwise; BTN straddler will act LAST after BB. UTG\'s button shape is unchanged from the non-straddle open case [CALL, RAISE, FOLD] (CALL = limp into the BTN straddle); the special_state machinery is downstream — getNextSeat must redirect last preflop action to BTN regardless of where action would normally close.',
  },
  {
    id: 'INV-S-015',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'BTN straddle, BB facing the extra blind (BTN=8 has posted, all seats UTG..CO and SB folded around). CHECK is illegal because BB is facing a $2 effective bet. CRITICAL: isBBOption transform fires today (no `raise` action recorded), so harness produces [CHECK, RAISE, FOLD] — same bug class as INV-S-008 but for BTN scope. Production fix: isBBOption must recognize a posted straddle (UTG or BTN) as the effective last raise — not just check for absence of a raise action.',
  },
  {
    id: 'INV-S-016',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'BTN straddler last-action option (action came back around: UTG..CO folded, SB called, BB called, returns to BTN). Symmetric to INV-S-012 (UTG straddler option) but for BTN scope. Confirms the straddler-option transform must work for BOTH allowed straddle seats — the production guard must key off "is hero the straddler", NOT "is hero seat 3" or "is hero in BB seat".',
  },
  {
    id: 'INV-S-017',
    ...
    status: 'spec_gap',
    bug_id: null,
    comment: 'PRECEDENCE pin (owner decision 2026-05-02): game config = optional, BOTH UTG (seat 3) and BTN (seat 8) seat occupants opted to straddle. Per the rule UTG > BTN, only UTG straddle posts; BTN does NOT straddle this hand. Pot = SB + BB + UTG straddle (no BTN extra blind). UTG+1 (hero seat 4) is first to act; button shape identical to INV-S-007. This row exists so the future fix wave does NOT accidentally let both straddles post simultaneously when the config flag is set to "optional" or "either-with-utg-precedence".',
  },
