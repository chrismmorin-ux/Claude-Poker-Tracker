/**
 * TournamentBridgeContext.jsx — Tournament auto-detection + state sync with extension
 *
 * Detects tournaments from extension data (seatDisplayMap or ante > 0),
 * auto-initializes the tournament engine, and pushes tournament state
 * (M-ratio, blind-out, ICM) back to the extension.
 */
...
const TournamentBridgeContext = createContext(null);

const DEFAULT_LEVEL_DURATION_MIN = 15;

export const TournamentBridgeProvider = ({ children }) => {
  ...
  // No value needed — this context is a side-effect-only provider
  return (
    <TournamentBridgeContext.Provider value={null}>
      {children}
    </TournamentBridgeContext.Provider>
  );
};
