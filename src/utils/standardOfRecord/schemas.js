...
const DECISION_RECORD_FIELDS = [
  ...
  { name: 'heroSeat', type: 'string|number|null', since: 1, required: false,
    note: 'Positional decomposition — which seats pay. MEASURED on real corpus rows as a numeric STRING ("6") while buttonSeat arrives numeric — the union type records the parser\'s actual output, not the wish.' },
  { name: 'buttonSeat', type: 'string|number|null', since: 1, required: false,
    note: 'With heroSeat, derives relative position from raw state instead of storing a position label.' },
  { name: 'opponentSeat', type: 'string|number|null', since: 1, required: false,
    note: 'Villain identity axis (numeric string on corpus rows, like heroSeat). Present since WS-393 — WS-410 §4.3\'s claim that it is discarded is corrected by this line.' },
