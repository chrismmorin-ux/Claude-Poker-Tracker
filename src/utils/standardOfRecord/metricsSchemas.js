...
const DEPTH_ABLATION_FIELDS = [
  ...
  { name: 'flipCountByStreet', type: 'object', since: 1, required: true, unit: 'map (street → {flips, n})',
    note: 'The counts behind the shares. Legacy k/n dialect ({flips, n}) — Stage 2 adds the canonical conditioned-rate alias beside it.' },
...
const PER_PLAYER_WIDTH_FIELDS = [
  ...
  { name: 'headlineSe', type: 'number|null', since: 1, required: true, unit: 'nats/decision', note: 'SE of the headline. Stage 2 adds the unit-suffixed alias.' },
  ...
  { name: 'headlineCiLow', type: 'number|null', since: 1, required: true, unit: 'nats/decision', note: 'CI low. Stage 2 adds the suffixed alias.' },
  { name: 'headlineCiHigh', type: 'number|null', since: 1, required: true, unit: 'nats/decision', note: 'CI high.' },
...
const RIVER_FLIP_REPLICATE_FIELDS = [
  ...
  { name: 'systematicFlipShare', type: 'number', since: 1, required: true, unit: 'share [0,1]',
    note: 'Share of river decisions whose top action differs between the arms in EVERY one of R seeded replicates — systematic, not seed noise.' },
  { name: 'systematicFlipCiLow', type: 'number|null', since: 1, required: true, unit: 'share [0,1] (player-clustered)',
    note: 'The QUOTABLE interval, clustered on players as clusterUnit promises. Stage 2 adds unit-suffixed aliases so the two interval families stop being distinguishable only by suffix.' },
  { name: 'systematicFlipCiHigh', type: 'number|null', since: 1, required: true, unit: 'share [0,1] (player-clustered)', note: 'Quotable interval, high.' },
  { name: 'systematicFlipCiLowBinomialOverDecisions', type: 'number', since: 1, required: true, unit: 'share [0,1] (binomial over decisions)',
    note: 'Within-node-set precision — named separately so it can never be mistaken for the generalising interval.' },
  { name: 'systematicFlipCiHighBinomialOverDecisions', type: 'number', since: 1, required: true, unit: 'share [0,1] (binomial over decisions)', note: 'Same, high bound.' },
  { name: 'seedDependentShare', type: 'number', since: 1, required: true, unit: 'share [0,1]',
    note: 'Decisions that flip in SOME replicates — the seed-noise mass, separated from the systematic signal.' },
  { name: 'singleEvaluationFlipShare', type: 'number', since: 1, required: true, unit: 'share [0,1]',
    note: 'What a single unreplicated evaluation would have reported — the number this instrument exists to correct.' },
...
const STUDY_LADDER_FIELDS = [
  ...
  { name: 'axes', type: 'object', since: 1, required: true, unit: 'map (axisId → axis block)',
    note: 'Per-axis separability evidence. Carries the RENAMED conditioning dialect ({conditioning, numerator, evalK, evalN, evalRate}) — Stage 2 adds the canonical conditioned-rate alias inside each block.' },
...
const FOLD_CURVE_SHAPE_FIELDS = [
  ...
  { name: 'fit', type: 'object', since: 1, required: true, unit: 'record ({partition, n, k, marginalFoldRate})',
    note: 'POOL-days fit set. Carries the {n, k, marginalFoldRate} dialect — Stage 2 adds the canonical conditioned-rate alias beside it.' },
