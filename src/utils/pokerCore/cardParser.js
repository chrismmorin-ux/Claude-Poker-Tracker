/**
 * cardParser.js - Card string to numeric form for fast comparison
 *
 * Parses card strings like "A~" to numeric encoded integers (0-51).
 * Reuses RANKS/SUITS from gameConstants.
 */

import { RANKS, SUITS } from '../../constants/gameConstants';

// Rank values: 2=0, 3=1, ... A=12
const RANK_VALUES = {};
for (let i = 0; i < RANKS.length; i++) {
  RANK_VALUES[RANKS[i]] = 12 - i; // RANKS is A,K,Q,...2 so reverse
}

const SUIT_VALUES = {};
for (let i = 0; i < SUITS.length; i++) {
  SUIT_VALUES[SUITS[i]] = i;
}

export const parseCard = (str) => {
  if (!str || str.length < 2) return null;
  const rankChar = str[0];
  const suitChar = str.slice(1);
  const rank = RANK_VALUES[rankChar];
  const suit = SUIT_VALUES[suitChar];
  if (rank === undefined || suit === undefined) return null;
  return { rank, suit };
};

export const encodeCard = (rank, suit) => rank * 4 + suit;

export const cardRank = (encoded) => (encoded >> 2);

export const cardSuit = (encoded) => (encoded & 3);

export const parseAndEncode = (str) => {
  const parsed = parseCard(str);
  if (!parsed) return -1;
  return encodeCard(parsed.rank, parsed.suit);
};

export const parseBoard = (communityCards) => {
  if (!Array.isArray(communityCards)) return [];
  return communityCards
    .filter(c => c && c !== '')
    .map(parseAndEncode)
    .filter(c => c >= 0);
};

/**
 * Get community cards visible up to a given street.
 */
export const getCardsForStreet = (communityCards, street) => {
  if (!communityCards) return [];
  const filled = communityCards.filter(c => c && c.trim().length >= 2);
  switch (street) {
    case 'preflop': return [];
    case 'flop': return filled.slice(0, 3);
    case 'turn': return filled.slice(0, 4);
    case 'river': return filled.slice(0, 5);
    default: return filled;
  }
};

export const TOTAL_CARDS = 52;
export { RANK_VALUES, SUIT_VALUES };
