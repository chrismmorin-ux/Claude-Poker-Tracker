...
import { tryParseSituationKey, weaknessMatchesSituation, describeSituation } from '../pokerCore/situationKey';
...
export const matchHeroWeakness = (situationKey, heroWeaknesses) => {
  ...
  // WS-318: matching is `weaknessMatchesSituation` — street + isAgg + contextAction (founder
  // decision, rule (c)). This site previously compared street + isAgg only, because it read
  // the key positionally as `const [actionStreet, , , actionType] = ...` and axis 3 is
  // `isAgg`, not an action. See WEAKNESS_MATCH_AXES for the decision and the measured impact.
  //
  // tryParse, not parse: the pre-WS-317 code never threw on a malformed key, and a display
  // helper must not start crashing its callers over one. An unparseable key matches nothing.
  ...
  for (const weakness of heroWeaknesses) {
    ...
    const match = weakness.situationKeys.some(sk => weaknessMatchesSituation(sk, parsedAction));

    if (match) {
      return {
        weakness,
        matchCount: weakness.sampleSize || 1,
        message: `Pattern: ${weakness.label} — occurrence #${weakness.sampleSize || 1} ${describeSituation(parsedAction)}`,
      };
    }
  }
