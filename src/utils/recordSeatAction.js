...
import { getCurrentBet, getSeatContributions } from './potCalculator';
 ...
 * @returns {{ type: string, payload: { seat: number, action: string, amount?: number } }}
 *
 * @throws {TypeError}  Invalid arguments (non-integer seat, malformed intent,
 *                      raiseTo not finite-positive-number, etc.)
 * @throws {RangeError} raiseTo <= effective currentBet (illegal raise sizing)
 */
export const buildSeatActionPayload = (gameStateView, seat, intent) => {
  ...
  // MATCH — auto-resolves to CHECK or CALL based on amountOwed.
  if (intent === 'match') {
    ...
    if (amountOwed === 0) {
      return {
        type: GAME_ACTIONS.RECORD_PRIMITIVE_ACTION,
        payload: { seat, action: PRIMITIVE_ACTIONS.CHECK },
      };
    }
    return {
      type: GAME_ACTIONS.RECORD_PRIMITIVE_ACTION,
      payload: { seat, action: PRIMITIVE_ACTIONS.CALL, amount: amountOwed },
    };
  }

  // RAISE — { raiseTo: number }
  if (intent && typeof intent === 'object' && 'raiseTo' in intent) {
    const { raiseTo } = intent;
    ...
    return {
      type: GAME_ACTIONS.RECORD_PRIMITIVE_ACTION,
      payload: { seat, action: label, amount: raiseTo },
    };
  }

  throw new TypeError(
    `buildSeatActionPayload: unrecognized intent. Expected 'fold' | 'match' | { raiseTo: number }, got ${JSON.stringify(intent)}`
  );
};
