...
Last updated: 2026-06-12 (SPR-122 closed — WS-226 shared IDB transaction helpers: readTx/writeTx/updateTx/cursorTx/atomicTx collapse 95/97 hand-rolled sites behind unchanged signatures; durable-writes decision (writes resolve on tx.oncomplete). Commit 77d065e. Also queued WS-225/227 from refactor-candidate scan.)
...
|------|--------|-------|--------|
| Tests | GREEN | `bash scripts/smart-test-runner.sh` | 12,349 app passing (507 files; 20 skipped; 1 todo; +19 via SPR-122 dbTransactions suite) + 2,321 extension (79 files). Known load flakes only (WS-220 RangeLabExplorer; gameTreeDepth2; AnchorObservationModal timeout once 2026-06-12) — all green isolated |
| Build | GREEN | `npm run build` | Vite production build clean (9.97s; PWA 66 entries) |
| Git | GREEN | `git status --short` | SPR-122 committed `77d065e` (2026-06-12) on `main` |
...
| Metric | Value | Notes |
|--------|-------|-------|
| Test count (app) | ~12,330 | Vitest (506 files; 20 skipped; 1 todo) |
| Test count (extension) | 2,321 | ignition-poker-tracker (79 files) |
