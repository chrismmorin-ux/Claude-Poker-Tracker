...
Last updated: 2026-06-11 (SPR-121 closed — WS-217 extension WS replay seam test; caught + fixed HUD-drop fold bug (SW validation gate vs HSM seat semantics). Commit 000f4a5.)
...
|------|--------|-------|--------|
| Tests | GREEN | `bash scripts/smart-test-runner.sh` | 12,330 app passing (506 files; 20 skipped; 1 todo) + 2,321 extension (79 files, +10 via SPR-121). Known load flakes only (WS-220 RangeLabExplorer; gameTreeDepth2 once) — both green isolated |
| Build | GREEN | `npm run build` | Vite production build clean |
| Git | GREEN | `git status --short` | SPR-121 committed `000f4a5` (2026-06-11) on `main` |
