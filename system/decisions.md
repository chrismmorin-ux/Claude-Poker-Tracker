...
### DEC-030: Preflop range generation executes the GUT charter; v1 is RFI-only
**Date:** 2026-08-04 | **Status:** Accepted | **Detected:** implicit
**Decision:** The "generate our own preflop ranges" capability is not a new architecture — it executes `.claude/projects/gut-range.md` (chartered 2026-07-29, never started), amended for three requirements the charter lacks: three solved stack depths (30/100/200bb), a rec-vs-studied-reg axis, and published charts as a scoring guide. v1 covers **RFI only**, truncating at the 4-bet with a documented fixed continuation. The pinned-subgame ladder (4bp → 3bp → SRP → limped) and WS-270 are deferred to v2.
**Reasoning:** The charter's thesis — "an action frequency and the range that takes it are the same fact stated twice" — is correct, and its measured defect analysis (grids and scalars disagreeing by up to 12×, scenario sums of 0.777 and 1.154) is the reason the work is worth doing. Re-deriving an architecture beside it would duplicate it. RFI-only keeps v1 shippable: the ladder requires a solved 4-bet terminal that does not exist, and pinning an unverified authored value would propagate a guess down every rung with the authority of a solve.
**Trade-off:** v1 does **not** answer the BTN-vs-CO 3-bet question that started the thread — that is a facing-a-raise spot. Accepted deliberately; first candidate for v2.
**Context:** Founder approval 2026-08-04. Prerequisites P-1a/P-1b shipped (`d4339a5`, `1be6d9d`). Plan at `/root/.claude/plans/reactive-singing-marble.md`.

### DEC-031: τ is the rec-vs-reg axis; polarization is derived, never labelled
**Date:** 2026-08-04 | **Status:** Accepted | **Detected:** implicit
**Decision:** The recreational-vs-studied-regular distinction is modelled as the GUT charter's existing `τ` — the softmax selection sharpness in `P(a|h) = softmax((EV_a + b_a) / τ)` — estimated **per villain** rather than per pool. Polar-vs-linear range shape is an **output** of τ, never an authored label or a discrete archetype.
**Reasoning:** Low τ means sharp selection: the player 3-bets only where 3-betting is clearly best, so the middle hollows out and the range reads polarized. High τ smears the boundaries into a linear/merged shape. This satisfies §7.2 (labels are outputs, never inputs) by construction rather than by discipline, and it replaces the current practice in `populationPriors.js:383-453`, where shape is baked into unnamed threshold constants and bluff-tail literals. Empirically supported: `docs/research/player-archetypes-empirical-2026-07-26.md` found silhouette twice as good at k=2 as any other k over 1,390 players — the six authored style labels are not in the data, so a continuous scalar anchored to the two measured poles is the honest representation.
**Trade-off:** `splitsRangePreflop` is a tempting τ signal and a bad one read naively — high τ mixes everywhere, while a studied player mixes only near indifference. The discriminating test is *where* they mix, not *whether*. Not to be wired in without that conditioning.
**Context:** Founder direction 2026-08-04: "the studied regular will have advanced behavior patterns that will imply their knowledge level and therefore if they polarized or are linear."

### DEC-032: Generated ranges are a prior, not a lookup
**Date:** 2026-08-04 | **Status:** Accepted | **Detected:** implicit
**Decision:** Generated preflop ranges ship as a committed artifact (`preflopSolvedPriors.js`, same pattern as `preflopEquityTable.js`) that serves as the **unobserved-villain prior**. The live per-combo computation stays the canonical path whenever the seat is known.
**Reasoning:** Mirrors the WS-274 architecture already in place for fold rates, and keeps §7.6's ban on bucket-keyed lookups satisfied — a committed table consulted *in the decision path* would be exactly the anti-pattern. It also matches §2.1's existing treatment of opening charts as "the unobserved-seat prior, not the answer." A pure-lookup design was rejected for that reason; a pure-live design was rejected because a 169-hand sweep costs ~15s per configuration and cannot meet a live budget.
**Context:** Founder selection 2026-08-04 from three options.

### DEC-033: Published charts score as a guide with a divergence registry, never as a gate
**Date:** 2026-08-04 | **Status:** Accepted | **Detected:** implicit
**Decision:** Generated ranges are scored against published charts (PokerCoaching, GTO Wizard reference), but divergence never fails a build. Every material divergence must be **explained and recorded**, extending the POKER_THEORY §9 documented-divergence pattern. The real metric stays the WS-293 discrimination probe — mean log P of the hand actually held.
**Reasoning:** Founder premise, and it is correct: we are not playing against the solver, so its output is not authoritative. A pass/fail gate against published charts would encode the opposite and cap the tool's ceiling at the thing it is trying to beat. Divergence is the product; **unexplained** divergence is the bug. §9 already has five documented divergences and the machinery to hold more.
**Context:** Founder selection 2026-08-04. Note BTN_vs_CO is currently an *undocumented* divergence — `archetypeRanges.js` claims a GTO Wizard/PokerCoaching baseline while running noticeably tighter, and §9 does not cover it.

### DEC-034: Confidence propagates as the minimum along the path
**Date:** 2026-08-04 | **Status:** Accepted | **Detected:** implicit
**Decision:** Any pinned-subgame propagation must carry `(estimate, effective-n)` rather than a point verdict, and a read's reported confidence is the **minimum along its propagation path**, not the confidence at the node being displayed. Hard pins at 0%/100% are forbidden; pin strength is capped by the consequence of the pin being wrong.
**Reasoning:** Founder objection, and it is sharper than it first appears. Three mechanisms compound: (1) best-response is a maximizer, so it systematically selects actions whose EV was *over*estimated — pinning removes the hedging that honest uncertainty buys; (2) error propagates upward and is re-amplified at every rung, since each level best-responds to the level below; (3) a 0%/100% node lock is the same mathematics as WS-302's zeroed grid cell — "a starting belief of zero is not a prior, it is a permanent verdict" — a lesson this repo already paid for one layer down. Two uncertainties must also stay separate: estimation error shrinks as `n/(n+PRIOR_WEIGHT)`, but **execution variance never shrinks at any n**, because a measured frequency is an aggregate over player states. The existing consequence-weighted gate is local, so a thin pin several rungs down can currently feed a read that presents as confident on its own local evidence.
**Trade-off:** Soft pins weaken the propagation effect the ladder exists to exploit — the reshaping is proportional to how hard you may pin. Accepted: the alternative is an unfalsifiable verdict that can turn the strategy negative-EV precisely where it was most confident.
**Context:** Founder 2026-08-04: "what if the node lock you just did on that 4bet fold is wrong? ... this would be where we give a disclaimer on the read." Binds the v2 ladder; to be written into `gut-range.md` before any of it is built.

### DEC-035: parseRangeString rejects unrecognized tokens
**Date:** 2026-08-04 | **Status:** Accepted | **Detected:** implicit
**Decision:** `pokerCore/rangeMatrix.parseRangeString` throws on any token it cannot express, rather than skipping it. Weight-suffix handling stays lenient, as pinned by existing tests.
**Reasoning:** A silently dropped token produces a **truncated range that is indistinguishable from a deliberately narrow one**, which is why 22 dash tokens in `archetypeRanges.js` survived for months with a green suite — `BB_vs_BTN` was defending the button with 244 combos missing, including every offsuit ace. Strictness also caught a bare `AK` silently setting `AA`, and two `bucketTaxonomy` tests that passed specific-suit combo notation to a hand-class parser and had been asserting over zero combos. `rangeLabPersistence` is the only caller touching external data and already wraps parsing in try/catch returning null, so throwing degrades safely.
**Trade-off:** Strict tokens with lenient weights is an inconsistent contract. Accepted rather than break two deliberately-pinned tests; the silent-drop risk lives in tokens, not weights.
**Context:** GUT Range P-1a, commit `d4339a5`.
