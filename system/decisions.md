...
### DEC-030: Founder hand reviews live in docs/hand-reviews/ with a paired reproducible harness
**Date:** 2026-08-07 | **Status:** Accepted | **Detected:** implicit
**Decision:** A founder-requested hand review is delivered as a markdown writeup in `docs/hand-reviews/` (dated, one file per hand) plus a vitest harness whose heavy compute parts are gated behind an env flag (`HAND_EVAL=1`) so only the fast record-validation part runs in the normal suite. Analysis numbers must come from the app's own engines (`bestFiveFromSeven` enumeration, `handVsRangesMW`), not ad-hoc math.
**Reasoning:** The review is durable and re-runnable instead of chat-transcript-only; the record-validation half doubles as a permanent regression test for straddle-first, all-in flags, and per-pot winner attribution through the real reducers; and using the app's engines means any engine bug surfaces as a wrong review number — a free cross-check. Gating keeps ~45s of CPU out of the 13k-test suite.
**Context:** 2026-08-07 remote session — AKs squeeze 4-way review (`docs/hand-reviews/2026-08-07-aks-squeeze-4way.md`, `src/test/aksSqueezeHandEval.test.js`).

### DEC-031: Single-hand entry never goes through importAllData
**Date:** 2026-08-07 | **Status:** Accepted | **Detected:** implicit
**Decision:** Getting one hand into the founder's on-device record is manual entry through the app UI (sequence documented per hand). The backup import path (`exportUtils.importAllData`) must never be used for it, and AI sessions must never hand the founder a single-hand JSON framed as importable.
**Reasoning:** `importAllData` calls `clearAllData()` first — it is a full restore, not a merge. A single-hand "import" would silently erase the founder's entire hand history. A merge-import is a candidate future item if this friction recurs; until then the safe path is the only path. Also: cloud sessions cannot reach the device's IndexedDB at all, so deferred on-device entry tasks (queue items like WS-321) are the correct handoff shape.
**Context:** 2026-08-07 remote session — discovered while looking for a way to enter the reviewed hand remotely; WS-321 filed as the deferred entry task.
