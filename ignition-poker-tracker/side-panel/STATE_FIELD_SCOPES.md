...
- `journalHealth` — WS-515: durable-journal bytes-in-use, entry count, and the permanent-loss counters (`dropped`, `quotaFailures`). Session-level and deliberately NOT cleared on table switch: hand loss is a property of storage, not of which table is seated, and clearing it would erase the only founder-visible evidence that hands have no durable copy. Refreshed by the registered `journalHealth` interval (60s) plus one read at boot. `null` until the first read — `renderStorageWarning` shows nothing while unknown, because unknown is not evidence of loss.
