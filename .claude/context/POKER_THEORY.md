...
**`FOUR_BET_FREQUENCIES` is VESTIGIAL as of the WS-521 follow-up (§2.5.5a) — the engine does not read it.** Corrected 2026-08-20: it was the only measured frequency table in `populationPriors.js` when this paragraph was written, and the sentence outlived the fix twenty lines below it. `FACED_3BET_FREQUENCIES_BY_ROLE` is now also measured (n = 6,531 over three roles) and is what prices this tree; `FOUR_BET_FREQUENCIES` has **zero production readers** (`LBL-four-bet-frequencies`, `liveness: vestigial`, `readSites: 0`). **Do not delete it.** `populationPriors.test.js` uses it as the INDEPENDENT CROSS-CHECK that licenses the `cold` and `passive` rows, which have no other reference — the opener row reproduces it to under 1pp on all three actions from a different pipeline. Its values — from `HANDHQ_OPENER_FACING_3BET` (`full` seat bucket, n = 398,577): fold 0.4337 / call4 0.4450 / fourBet 0.1213 — carry two conditionals that must be quoted alongside them:
