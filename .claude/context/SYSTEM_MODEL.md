...
| `pokerCore/` (14 modules) | Shared poker primitives (cards, ranges, hand eval, board texture, exact preflop equity), the canonical situation key (WS-317), and the holding-knowledge join (WS-292) | Imported by both engines; imports from neither. **`holdingKnowledge.js` is the single accessor joining an inferred range to the hand a seat turned out to hold** — revealed cards are a SCORING channel, gated behind `revealedHolding()` and structurally absent from the live path (asserted in `gameTreeContext.test.js`) |
