...
verified-by: refactor-sprint-2026-05-10
staleness-threshold-days: 30
---

# System Model — Poker Tracker
**Version**: 2.0.0 | **Updated**: 2026-05-10 | **Owner**: Refactor Sprint 2026-05-10 (architectural reset, Item 1 of 5)
...
**Restoration note (2026-05-10):** This file was missing from the canonical location for an unknown period. Restored from `.pre-cwos-backup` and updated for current state. Counts + decision-system pattern (§6.1 C-15) + 3 new tech debt items (TD-15/16/17) + new decision log entry (2026-05-10) are this session's deltas. Sections marked **[verify-pending]** were not re-checked against code this session — Item 1 continuation in the next session will deepen verification.
...
| **13 Reducers** (was 8 on 2026-04-21; +5 since — full list **[verify-pending]**) | State transitions | Pure functions, no side effects, no async |
| **20 Context Providers** (was 13; +7 since 2026-04-21 — covers PEO/PRF/EAL/HSP/PMC/SCF program additions) | Cross-component state distribution | Each wraps one reducer + optional persistence hook |
| **62 Hooks** (was 34; nearly doubled since 2026-04-21 — flag for survey/possible split candidates) | Business logic, side effects, computed values | May read multiple contexts; never dispatch to foreign reducers |
...
| **`persistence/` (29 modules)** | **IndexedDB v27**, 23 stores, migrations | Sole interface to IndexedDB; all access through exported functions. **Migration registry** at `migrationRegistry.js` (single source of truth: per-version description, storesAdded/Changed, owner). `getStoreOwner(name)` + `getVersionsForStore(name)` helpers. CI gate `scripts/check-idb-additive.sh` forbids `deleteObjectStore`/`deleteIndex` in `migrations.js`. Resolves TD-16. **Shared transaction helpers (WS-226, 2026-06-12):** `readTx`/`writeTx`/`updateTx`/`cursorTx`/`atomicTx` in `database.js` collapsed 95 of 97 hand-rolled `db.transaction` sites. Contract: helpers reject with the RAW DOMException (never wrapped — `error.name` checks survive); helpers never log (modules' `createPersistenceLogger` try/catch owns logging + swallow-vs-propagate); **write helpers resolve on `tx.oncomplete` (after durable commit), not `request.onsuccess`** (DEC: WS-226 durable-writes). Documented exceptions stay hand-rolled: `handsStorage.batchUpdateSeatPlayers` (bespoke abort-tracking) + `playersStorage.deletePlayer` (parallel cursor cascade). Helper suite: `__tests__/dbTransactions.test.js`. |
| **`heroState/` (NEW 2026-05-03)** | Hero state primitive: types + classifier + role-translator + builder | Pure data; consumed by HandReplay. 18 archetype templates at `docs/design/hero-state-templates/`. Archetype IDs are OUTPUTS, never INPUTS to plan computation. |
| **`holdingKnowledge/` (NEW 2026-08-01, WS-292)** | The villain-side counterpart of `heroState/`: what we KNOW about a seat's holding at a decision point — inferred range, ground truth from showdown, and narrowing provenance as ONE value. `openHolding`/`narrowHolding`/`revealHolding`/`holdingBelief`/`holdingTruth`. | **THE single derivation of "the range at this point."** All 6 production narrowing sites route through it (`decisionAccumulator`, `gameTreeContext`, `gameTreeEvaluator` refinement, 5× `gameTreeDepth2`, `replayAnalysis` by DI, `liveAdvisor.narrowWithLog`) plus the corpus probe. Two binding rules: (1) revealed cards NEVER reach the range for the decision being scored — enforced by a same-reference test — but always feed the player's model afterwards; (2) `basis` (`observed` \| `hypothesized`) is REQUIRED, and `holdingTruth` refuses hypothesized ranges because scoring a counterfactual branch against a revealed hand is a category error. Provenance makes WS-303's narrowing-count invariant a property of the value (`computeDepth3BarrelEV` returns `maxNarrowingCount`, must be 2). Migration verified bit-identical: corpus probe unchanged on every pre-existing key, 8/8 engine EV scenarios identical under frozen clocks. Rules: `src/utils/holdingKnowledge/CLAUDE.md`. |
| **`decisionSystems/` (NEW 2026-05-14, SPR-078)** | Shared infra for stateful decision-systems: accumulator (Beta-Binomial + Wilson, canonical Z_95) + registry (append-only) + idbStore (3 factories + migrationGuard) + reducerComposition (slice helpers) | Day-1 member: `anchorLibrary/` (migrated). Tracked-as-future: `assumptionEngine/`. Excluded: `skillAssessment/` + `predictionAudit/` (pure detectors, different pattern). ADRs at `.claude/decisions/2026-05-14-*`. |
| **Decision-system instances** — `assumptionEngine/`, `anchorLibrary/`, `skillAssessment/`, `predictionAudit/` | Stateful: assumptionEngine + anchorLibrary (5-piece pattern). Pure-detector: skillAssessment + predictionAudit | anchorLibrary MIGRATED to `decisionSystems/` 2026-05-14 (primitiveValidity delegates to betaPosterior; 4 IDB wrappers collapsed; writers.js in-code mirror). assumptionEngine tracked-as-future per ADR-DS-1. See §6.1 C-15 + §11 TD-15. |
...
| **C-15: Stateful decision-system module pattern (4 instances → 1 shared spine)** | Shared spine: `src/utils/decisionSystems/` (SPR-078 / 2026-05-14). Members: `anchorLibrary/` (MIGRATED 2026-05-14) + `assumptionEngine/` (tracked-as-future). Excluded by design: `skillAssessment/` + `predictionAudit/` (pure-detector pattern, separate module if shared infra needed). | Shared infra: `accumulator/` (Beta-Binomial + Wilson primitives, canonical `Z_95 = 1.959963984540054`), `registry/` (append-only ID-keyed), `idbStore/` (3 factories + migrationGuard binding to `migrationRegistry`), `reducerComposition/` (slice helpers, unadopted by anchorLibrary). | Charter (`refactor-sprint-2026-05-10.md` Item 4) ratified extraction. ADR-DS-1 (`.claude/decisions/2026-05-14-decision-systems-extraction.md`) governs the boundary; ADR-DS-2 binds IDB factories to `migrationRegistry`. anchorLibrary migration: `primitiveValidity.js` delegates Beta math to `betaPosterior`; 4 IDB store wrappers collapsed (~600 LOC deleted); `writers.js` in-code mirror of `WRITERS.md` shipped. **Status:** partially resolved — 1 of 2 stateful members migrated. Remainder tracked as TD-15-followup. |
...
| **TD-15** | **PARTIALLY RESOLVED 2026-05-14 (SPR-078).** Decision-system pattern extracted to `src/utils/decisionSystems/` per Refactor Sprint Item 4. 4-piece surface shipped: accumulator (Beta-Binomial + Wilson primitives, canonical Z_95) + registry (append-only ID-keyed) + idbStore (3 factories + migrationGuard) + reducerComposition (slice helpers, unadopted). **anchorLibrary migrated as proof** — `primitiveValidity.js` delegates Beta math; 4 IDB store wrappers collapsed (~600 LOC deleted); `writers.js` in-code mirror of WRITERS.md shipped. ADR-DS-1 (`.claude/decisions/2026-05-14-decision-systems-extraction.md`) governs the boundary. **assumptionEngine, skillAssessment, predictionAudit excluded by design** (skillAssessment + predictionAudit are pure-detector pattern, not stateful decision-systems). Remaining work tracked as TD-15-followup. | **Partially resolved.** | Refactor Sprint architectural survey 2026-05-10. Owner-ratified extraction approach 2026-05-14 via plan-mode Q1=Broader / Q2=Bind / Q3=Two ADRs. | SPR-078 closeout. |
| **TD-15-followup** | **assumptionEngine migration to `decisionSystems/`.** assumptionEngine still inlines its own Beta math (z=1.96 vs Z_95) + builds its own VillainAssumptions reducer + bespoke IDB wrapper at `assumptionStorage.js`. Migration is deferred per ADR-DS-1 "tracked-as-future." Forcing function: next time assumptionEngine touches its persistence layer OR PMC Phase 5b ships (whichever first). | Low-Medium | SPR-078 closeout 2026-05-14 | Migrate during whichever forcing-function event fires first. ADR amendment if shape mismatch surfaces. |
| **TD-16** | **RESOLVED 2026-05-11 + 2026-05-14.** Item 3 (2026-05-11) built migration registry at `src/utils/persistence/migrationRegistry.js` documenting all 25 versions with per-version description + storesAdded/storesChanged + owner. Tests at `__tests__/migrationRegistry.test.js` (19 cases) enforce completeness, additive-only semantics, monotonic store-set growth, owner-program integrity, and DB_VERSION alignment. Companion CI gate `scripts/check-idb-additive.sh` forbids `deleteObjectStore`/`deleteIndex` in `migrations.js`. Wired into `scripts/smart-test-runner.sh`. **Item 4 sub-resolution (2026-05-14):** ADR-DS-2 closes the "stores can exist without registry entry" drift surface — `decisionSystems/idbStore/migrationGuard.assertStoreRegistered` is called at every factory construction; an unregistered store throws synchronously at import. Test escape hatch: `__testing__.bypassMigrationCheck` (test-only). **Note:** actual store count is 23 (not "~30"); registry is the canonical count source. | Refactor Sprint Items 3 + 4. | Resolved. |
| **TD-17** | **RESOLVED 2026-05-14 (SPR-080 Item 1).** `useLiveActionAdvisor.js` split: 6 pure helpers extracted to `src/utils/liveAdvisor/computeHelpers.js`; hook slimmed from 528 → 370 LOC. PMC Phase 5b integration point shipped as `onHandComplete` callback option (fires before `setAdvice` with `{ handNumber, street, heroCards, villainSeat, prediction, modelVersion }`). All 3301 surrounding tests stay green. Other top-5 hooks (`usePlayerFinder` 466, `useSyncBridge` 386, `usePlayerFiltering` 359, `useSessionPersistence` 358) remain unchanged; tracked but no current forcing function. | Refactor Sprint Item 5 (SPR-080). Owner-ratified Q1=Helpers-out / Q2=src/utils/liveAdvisor/ / Q3=onHandComplete callback. | Resolved. |
| **TD-18** | **`.claude/workstream/sprint-index.yaml` has accumulated 70+ sprint entries without archival (~30K tokens, 845 lines).** Same canonical-file-too-big class as `system/state.md` (now reshaped). Still YAML-structured so individual entries are readable, but the full file exceeds the single-tool-call read limit. Not currently a session-orientation blocker (sprint-index is queried selectively, not read whole), but on a trajectory toward becoming one as sprint count grows. | Low-Medium | Refactor Sprint Item 2 spot-check 2026-05-10 | Defer: prune closed sprints older than ~90 days to `.claude/workstream/sprint-index-archive.yaml` once total exceeds ~50K tokens, OR introduce a slim index pointing at per-sprint files. Not in scope for current sprint. |
| **TD-19** | **Cursor error handlers in `migrations.js` don't call `tx.abort()`.** Multiple sites (e.g., L394-396 migrateV8, L425-427 migrateV10) log cursor errors but transaction continues. Root cause of FM-PERSIST-6 (migration partial-success state). Estimated ~10-15 cursor sites. | CRITICAL (silent partial migration state) | SPR-080 Item 2 / WS-188 Phase 1 (2026-05-14). See `docs/persistence/FAILURE_MODES.md` FM-PERSIST-6 + FM-PERSIST-12. | Add `tx.abort()` to every cursor `.onerror` handler in `migrations.js`. One PR; ~S effort. |
| **TD-20** | **Blob writers lack quota pre-check.** `savePhotoAtomically.js` doesn't call `navigator.storage.estimate()` before attempting large writes; quota-exceeded fires mid-transaction with unclear UX. | HIGH (mobile Safari ~50MB) | SPR-080 Item 2 / WS-188 Phase 1 (2026-05-14). See FM-PERSIST-3. | Add pre-check pattern to `savePhotoAtomically.js` + any future blob writers. Surface quota pressure to user. |
| **TD-21** | **Append-only writer enforcement is process-only (not runtime).** `anchorObservations` + `predictionAudit` are append-only by design (ADR-PERSIST-3) but IDB's `put()` allows overwrite. Writer-level discipline is enforced by registry + CI grep; no runtime guard rejects overwrite-on-put. | MEDIUM | SPR-080 Item 2 / WS-188 Phase 1 (2026-05-14). See FM-PERSIST-7 + ADR-PERSIST-3. | Consider `createAppendOnlyStore` factory in `decisionSystems/idbStore/` that rejects `put()` on existing key. Backwards-compatible with anchor + predictionAudit consumers. |
| **TD-22** | **TendencyContext lacks mode-switch invalidation.** Offline → online mode change doesn't explicitly invalidate the cached `tendencyMap`. Hand-count delta detect catches most cases, but offline-added-hands during paused-online state can miss the next poll. | MEDIUM | SPR-080 Item 2 / WS-188 Phase 2 (2026-05-14). See `docs/persistence/CACHE_CONTRACTS.md` §2 TendencyContext. | Add explicit invalidation hook on mode-switch. |
| **TD-23** | **OnlineAnalysisContext polling not driven by hand-arrived event.** 5s fixed-interval poll regardless of hand pace. Fast tables (turbo SnG, multi-tabling) can miss hand arrivals. | MEDIUM | SPR-080 Item 2 / WS-188 Phase 2 (2026-05-14). See `docs/persistence/CACHE_CONTRACTS.md` §2 OnlineAnalysisContext. | Drive invalidation from SyncBridge hand-arrived event in addition to (or instead of) 5s interval. |
| **TD-24** | **gameTree caches have no public factory or versioning contract.** `gameTreeEquity.js`, `gameTreeDepth2.js`, `gameTreeEvaluator.js` accept a caller-supplied cache parameter with no provenance check; caller can reuse a cache across model updates and silently get stale EVs. | HIGH (hot-path optimization with no safety net) | SPR-080 Item 2 / WS-188 Phase 2 (2026-05-14). See `docs/persistence/CACHE_CONTRACTS.md` §2 gameTree caches (CRITICAL GAP). | Expose `createGameTreeCache({ modelVersion })` factory that tags caches; engine checks tag before read; mismatch invalidates. |
...
| **2026-05-10** | **Refactor Sprint launched as deliberate pause on feature work.** 5-item program: (1) restore SYSTEM_MODEL.md from `.pre-cwos-backup` to canonical location, (2) build persistence migration registry (TD-16), (3) extract decision-system module pattern across 4 instances (TD-15/C-15), (4) split useLiveActionAdvisor 528-LOC hook (TD-17), (5) triage 34-item ready-to-start queue lake. Owner ratified **full architectural reset** (all 5 items) over staged sprints. Charter: `.claude/projects/refactor-sprint-2026-05-10.md`. | After ~6 weeks of heavy feature/design work (PIO/SCF/EAL/HSP/SHC/PRF/PMC/LSW), architectural survey surfaced 5 pressure points. SYSTEM_MODEL.md was missing from canonical location entirely; persistence layer at v25 with 30 stores had no governance; same 5-piece module pattern shipped 4× without extraction; one hook crossed 500 LOC. Each pressure point compounds with the next program added. Owner stance per memory `feedback_long_term_over_transition.md`: bias toward depth over short-term migration ergonomics. | Continue feature work + patch incrementally (rejected: pressure points compound with each program; survey identified the right moment because PMC just gave the smallest cleanest 4th instance to template against). Staged sprint with cheap wins first, defer extraction (rejected by owner: chose full reset). Item 5 (queue triage) before extraction (rejected: cheap, can run anytime; doesn't gate Item 3). |
| **2026-05-14** | **Refactor Sprint Item 4 SHIPPED (SPR-078).** Extracted `src/utils/decisionSystems/` with 4-piece surface (accumulator + registry + idbStore + reducerComposition). anchorLibrary migrated as proof: `primitiveValidity.js` delegates Beta math to canonical `betaPosterior` (Z_95 = 1.959963984540054); 4 IDB store wrappers collapsed via `createUpsertStore` (~600 LOC deleted); new `writers.js` in-code mirror of WRITERS.md via shared `createRegistry`. Two ADRs ratified: ADR-DS-1 (extraction charter) + ADR-DS-2 (IDB factory binds to migrationRegistry, closes TD-16 sub-item). 134 new tests + all 1590 anchorLibrary/persistence/decisionSystems tests green. assumptionEngine tracked as TD-15-followup. skillAssessment + predictionAudit explicitly excluded (pure-detector pattern). | Owner-ratified extraction approach 2026-05-14 via plan-mode Q1=Broader / Q2=Bind / Q3=Two ADRs. Long-term-aggressive doctrine + grounded-extraction-from-converged-code (z-constant drift was already real; store-wrapper boilerplate was 95% identical). Reducer-composition ships unadopted as forcing function for next stateful migration. | Q1=Narrow (rejected: doesn't address charter DoD, drift persists). Q1=Middle (rejected by ratified Q1=Broader vote: middle was valid but reducer-composition's cheap forcing-function value outweighed shipping it unadopted). Q2=Decouple (rejected: drift surface stays open). Q3=One ADR (rejected: future challenges to the binding shouldn't re-open umbrella decision). |
| **2026-05-14** | **Refactor Sprint 2026-05-10 CLOSED (SPR-080).** All 7 charter items shipped. Item 5: `useLiveActionAdvisor` split — 6 pure helpers extracted to `src/utils/liveAdvisor/computeHelpers.js`; hook slimmed 528 → 370 LOC; `onHandComplete` callback shipped for PMC Phase 5b integration. Item 6 (SPR-079): 13 ready-to-start backlog items recategorized + 2 promoted (WS-063, WS-066). Item 7 (WS-188): persistence + cache audit — 4 phase outputs (FAILURE_MODES.md 12 modes / CACHE_CONTRACTS.md 18 surfaces + 5 prose / 4 ADRs / CACHE_SWEEP_2026-05-14.md residual scan) + 5 INV-PERSIST-* invariants + 6 new TD entries (TD-19..24). Zero behavior-changing src/ diff for Item 7; documentation-only audit. | Owner-instruction "close out the refactor" 2026-05-14. Long-term-aggressive doctrine ratified the bundle. Multi-session over-cap sprint (~4-5 sessions) was the deliberate-program-close-out choice. The Refactor Sprint as a whole closes with: SYSTEM_MODEL.md restored to canonical location, state.md reshaped, persistence migration registry shipped + CI-gated, decisionSystems shared infrastructure extracted with anchorLibrary migrated, useLiveActionAdvisor split, queue triaged, persistence + cache audit complete. Project file (`.claude/projects/refactor-sprint-2026-05-10.md`) marked COMPLETED. | Continue feature work without Items 5/7 (rejected: PMC Phase 5b would have had to add to the 528-LOC hook; audit surfaces would have stayed undocumented). Single-item per sprint (rejected: would have spread close-out across 2-3 more sprints; owner explicitly asked for bundled close-out). |
