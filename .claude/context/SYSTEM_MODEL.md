...
| **`persistence/` (29 modules)** | **IndexedDB v25**, 23 stores, migrations | Sole interface to IndexedDB; all access through exported functions. **Migration registry** at `migrationRegistry.js` (single source of truth: per-version description, storesAdded/Changed, owner). `getStoreOwner(name)` + `getVersionsForStore(name)` helpers. CI gate `scripts/check-idb-additive.sh` forbids `deleteObjectStore`/`deleteIndex` in `migrations.js`. Resolves TD-16. |
