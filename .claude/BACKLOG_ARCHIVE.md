...
|----|-----|-------------|------------|
| RT-44 | P1 | renderKey fingerprint (appSeatData hash, exploit content hash, focusedVillainSeat ordering) | Closed as part of RT-43 completion (commit f080ae8). buildRenderKey now uses exploitPushCount + appSeatDataVersion + focusedVillainSeat-from-snapshot (snapshot is built atomically, so order is guaranteed). |
| RT-54 | P2 | renderKey: community cards + villain profile | Community cards are already in the key (`communityCards.filter(c=>c).join(',')` since prior commits). Villain profile coverage lands with commit f080ae8 via `appSeatDataVersion` (bumped on every exploits push that rebuilds appSeatData) + the new adviceFingerprint which includes advice.villainSeat for pinned-villain disambiguation. |
| RT-43 | P1 | Unified render scheduler + single-owner state store | The scheduler + coordinator already shipped in commit 892452e; the user-facing symptom of "sidebar shows wrong info for hand state" came from buildRenderKey skipping fields that UI displays (tournament data collapsed to a boolean; last-action fingerprint missed amount; advice fingerprint missed sizing and villainSeat). Commit f080ae8 fixes the fingerprint with content hashes for all three areas plus 7 regression tests locking the behaviour in. The 77 remaining direct DOM writes spot-check as legitimate Tier 1 (snapshot-driven via renderAll) or Tier 3 (event-driven: seat popover, copy feedback, user-triggered diagnostic panels). |
| RT-45 | P1 | Advice guard with hand-number binding | Commit 598a930: handleAdvicePush stamps the currently-locked handNumber onto each advice message; coordinator.handleAdvice rejects advice whose hand number differs from `_lockedHandNumber`. Catches the SW cache replay case where advice from HAND_A arrives after HAND_B started and the street-rank gap was within tolerance. Payloads without handNumber fall through to the existing street-rank guard. |
...
| RT-58 | P1 | Delete dead render functions + fix diag-dump bare-var refs | Commit f12c6f4: deleted computeFocusedVillain local wrapper (:905), renderBriefingPanel (117 lines), renderObservationPanel (73 lines), .briefing-item click handler (11 lines), and fixed the diagnostic-dump block at :2406–2427 to read from coordinator.get() instead of the five deleted module vars (lastHandCount, currentTableState, currentLiveContext, exploitPushCount, advicePushCount). The diagnostics copy button no longer risks throwing ReferenceError when a user reports a bug. |
| RT-59 | P1 | Route pipeline-status liveContext through handleLiveContext | Commit 6bf2cf7: replaced direct `coordinator.set('currentLiveContext', ...)` at side-panel.js:244 with `coordinator.handleLiveContext(ctx)`. Position-lock, `_receivedAt` injection, and pending-advice promotion now apply on both push paths. Mirror fix in test harness + Scenario 12 integration tests covering pipeline-status-only reconnect and pending-advice promotion across the unified path. |
| RT-60 | P1 | Coordinator timer registration contract | Commit fb4803b: RenderCoordinator exposes `registerTimer(key, handle, kind)` / `clearTimer(key)` / `clearAllTimers()` plus an `onTableSwitch(fn)` hook. Migrated all four module-level timer sites in side-panel.js (`tableGrace`, `tourneyTimer`, `staleContext`, `planPanelAutoExpand`) — last of which was the known post-RT-43 orphan-timer regression. `clearForTableSwitch()` + `destroy()` invoke `clearAllTimers()`. Seven new tests in render-coordinator.test.js cover replacement, cancellation, kind-aware clearing, lifecycle integration, and onTableSwitch hook tolerance. |
