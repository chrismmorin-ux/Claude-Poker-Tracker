...
| 2026-04-11 | YELLOW | R7 roundtable. RT-46 still open. Additional note: `count` value in pidCounts interpolated without escapeHtml; seatDisplayMap keys not validated for prototype pollution. Both low-priority in single-user context. No new backlog items. |
| 2026-04-20 | YELLOW | Drills Consolidation Roundtable. Three security-adjacent findings: (1) `aggregateFrameworkAccuracy` at `preflopDrillsStorage.js:108` uses `const out = {}` — tampered IDB record with `frameworks: ['__proto__']` could attempt prototype write (RT-96, trivial fix via `Object.create(null)`); (2) proposed `StudyView` dev-preview route needs explicit DEV+localhost gating otherwise production bundle ships preview code (RT-99); (3) proposed Learn tab unified search must render query highlights as React children (not `dangerouslySetInnerHTML`) to avoid NEV-11-class XSS in main app (RT-93 item g). All three pre-emptive — no confirmed vulnerability, single-user local context. |
