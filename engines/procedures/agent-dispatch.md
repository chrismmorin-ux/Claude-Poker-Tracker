...
3. When the agent returns, write to `<run_workspace>/phase-1/<agent-name>.yaml` using the artifact schema:
   ```yaml
   agent: "<agent-name>"
   ...
   ```
4. Update `manifest.yaml` artifacts map and `agent_matrix` after each agent completes.
...
Pass all Phase 1 artifact outputs, labeled by agent name. Write output to `<run_workspace>/phase-2/cross-critic.yaml` with `input_artifacts` listing the Phase 1 paths consumed. Update manifest.

### Error Handling

- If the cross-critic returns empty: write artifact with `status: failed`. Proceed to synthesis with Phase 1 artifacts only. Note in manifest: "Cross-critique failed — proceeding with Phase 1 artifacts only"
...
Write output to `<run_workspace>/phase-3/facilitator.yaml` with `input_artifacts` listing all consumed paths. Update manifest.
