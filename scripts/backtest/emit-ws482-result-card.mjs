...
const dealBook = await buildDealBook({
  ...
  // 120-file PREFIX, so the Deal Book must describe THAT sample; switching to proportional
  ...
  files: selectCorpusFiles(files, { maxFiles: 120, strategy: 'prefix' }).files,
  root: DEFAULT_CORPUS_ROOT,
  sliceSpec: { sites: null, stakes: ['50NLH'], maxFiles: 120, fileSelection: 'prefix' },
  identity: 'path+size',
});
