 ...
 *     ~0.37-0.41 player-weighted). Since WS-373 the basis TRAVELS: \`resolveStatPriors\`
 *     puts it on \`evidence.weighting\` and a consumer that receives an empirical prior
 *     with no declared basis refuses rather than assumes. WHICH basis should be served
 *     is still an OPEN FOUNDER DECISION — hand-weighted answers "the average hand dealt"
 *     and player-weighted answers "the average opponent"; they are different questions
 *     and the engine may need both. See the weighting block in \`poolBaseline.js\`.
