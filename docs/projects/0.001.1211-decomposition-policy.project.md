...
**Scale**: 57 atomic tasks across 10 phases, 49 files affected (18 new, 31 modified)
...
| 3 | [ ] | Decomposition Validation & Audit | 6 |
| 4 | [ ] | needs_context Protocol | 5 |
| 5 | [ ] | invariant_test Protocol | 5 |
| 6 | [ ] | CLAUDE_REQUEST_FOR_PERMISSION Protocol | 5 |
| 7 | [ ] | Recursive Decomposition | 4 |
| 8 | [ ] | Template & Documentation Updates | 11 |
| 9 | [ ] | Hook Updates & Integration | 5 |
| 10 | [ ] | Troubleshooting & Monitoring | 4 |
...
## Phase 3: Decomposition Validation & Audit <- CURRENT
...
| File | Purpose |
|------|---------|
| `scripts/audit_decomposition.cjs` | Validates all tasks meet atomic criteria |
| `.claude/audits/atomicity_report.json` | Output of audit |
| `.claude/hooks/decomposition-validator.cjs` | PreToolUse hook for validation |

### Files to Modify
| File | Changes |
|------|---------|
| `scripts/dispatcher.cjs` | Add pre-assign audit check |
...
| Task ID | Description | File | Model | Status |
|---------|-------------|------|-------|--------|
| T-P3-001 | Create audit_decomposition.cjs | `scripts/audit_decomposition.cjs` | deepseek | [ ] |
| T-P3-002 | Create decomposition-validator.cjs hook | `.claude/hooks/decomposition-validator.cjs` | deepseek | [ ] |
| T-P3-003 | Update dispatcher with pre-assign audit | `scripts/dispatcher.cjs` | qwen | [ ] |
| T-P3-004 | Register hook in settings.json | `.claude/settings.json` | qwen | [ ] |
| T-P3-005 | Write tests for audit script | `scripts/__tests__/audit_decomposition.test.js` | qwen | [ ] |
| T-P3-006 | Create sample atomicity_report.json | `.claude/audits/atomicity_report.json` | qwen | [ ] |

---

## Phase 4: needs_context Protocol
...
| File | Purpose |
|------|---------|
| `scripts/context-provider.cjs` | Extracts exact line ranges |

### Files to Modify
| File | Changes |
|------|---------|
| `.claude/schemas/local-task.schema.json` | Add needs_context field (if not in T-P1-001) |
| `scripts/execute-local-task.sh` | Support needs_context extraction |
| `scripts/dispatcher.cjs` | Handle context requests |

### Atomic Task Decomposition

| Task ID | Description | File | Model | Status |
|---------|-------------|------|-------|--------|
| T-P4-001 | Create context-provider.cjs | `scripts/context-provider.cjs` | deepseek | [ ] |
| T-P4-002 | Update schema with needs_context field | `.claude/schemas/local-task.schema.json` | qwen | [ ] |
| T-P4-003 | Update execute-local-task.sh for context | `scripts/execute-local-task.sh` | qwen | [ ] |
| T-P4-004 | Update dispatcher for context handling | `scripts/dispatcher.cjs` | qwen | [ ] |
| T-P4-005 | Write tests for context-provider | `scripts/__tests__/context-provider.test.js` | qwen | [ ] |

---

## Phase 5: invariant_test Protocol
...
| File | Purpose |
|------|---------|
| `scripts/invariant-test-generator.cjs` | Generates paired test tasks |
| `.claude/templates/invariant-test.template.js` | Template for invariant tests |

### Files to Modify
| File | Changes |
|------|---------|
| `.claude/schemas/local-task.schema.json` | Add invariant_test output field |
| `scripts/dispatcher.cjs` | Auto-create invariant_test tasks |

### Atomic Task Decomposition

| Task ID | Description | File | Model | Status |
|---------|-------------|------|-------|--------|
| T-P5-001 | Create invariant-test-generator.cjs | `scripts/invariant-test-generator.cjs` | deepseek | [ ] |
| T-P5-002 | Create invariant-test.template.js | `.claude/templates/invariant-test.template.js` | qwen | [ ] |
| T-P5-003 | Update schema with invariant_test field | `.claude/schemas/local-task.schema.json` | qwen | [ ] |
| T-P5-004 | Update dispatcher for auto-test creation | `scripts/dispatcher.cjs` | qwen | [ ] |
| T-P5-005 | Write tests for generator | `scripts/__tests__/invariant-test-generator.test.js` | qwen | [ ] |

---

## Phase 6: CLAUDE_REQUEST_FOR_PERMISSION Protocol
...
| File | Purpose |
|------|---------|
| `.claude/schemas/permission-request.schema.json` | Schema for escalation |
| `.claude/hooks/permission-request-handler.cjs` | Handles permission requests |
| `.claude/permission-requests.json` | Log of escalation requests |

### Files to Modify
| File | Changes |
|------|---------|
| `.claude/DECOMPOSITION_POLICY.md` | Document escalation protocol |
| `scripts/dispatcher.cjs` | Handle permission requests |

### Atomic Task Decomposition

| Task ID | Description | File | Model | Status |
|---------|-------------|------|-------|--------|
| T-P6-001 | Create permission-request.schema.json | `.claude/schemas/permission-request.schema.json` | deepseek | [ ] |
| T-P6-002 | Create permission-request-handler.cjs | `.claude/hooks/permission-request-handler.cjs` | deepseek | [ ] |
| T-P6-003 | Create permission-requests.json structure | `.claude/permission-requests.json` | qwen | [ ] |
| T-P6-004 | Update DECOMPOSITION_POLICY.md | `.claude/DECOMPOSITION_POLICY.md` | qwen | [ ] |
| T-P6-005 | Update dispatcher for permission handling | `scripts/dispatcher.cjs` | qwen | [ ] |
...
| File | Changes |
|------|---------|
| `scripts/dispatcher.cjs` | Re-submit failed tasks for decomposition |
| `.claude/hooks/decomposition-validator.cjs` | Track decomposition depth |

### Atomic Task Decomposition

| Task ID | Description | File | Model | Status |
|---------|-------------|------|-------|--------|
| T-P7-001 | Add REDECOMPOSE logic to dispatcher | `scripts/dispatcher.cjs` | deepseek | [ ] |
| T-P7-002 | Add decomposition depth tracking | `.claude/hooks/decomposition-validator.cjs` | qwen | [ ] |
| T-P7-003 | Add max-depth limits (prevent infinite recursion) | `scripts/dispatcher.cjs` | qwen | [ ] |
| T-P7-004 | Write tests for recursive decomposition | `scripts/__tests__/dispatcher-redecompose.test.js` | qwen | [ ] |

---

## Phase 8: Template & Documentation Updates
...
### Goal
All existing hooks comply with new policy.

### Files to Modify
| File | Changes |
|------|---------|
| `.claude/hooks/enforce-delegation.cjs` | Check for ///LOCAL_TASKS format |
| `.claude/hooks/delegation-enforcer.cjs` | Validate decomposition exists |
...
| Task ID | Description | File | Model | Status |
|---------|-------------|------|-------|--------|
| T-P9-001 | Update enforce-delegation.cjs | `.claude/hooks/enforce-delegation.cjs` | qwen | [ ] |
| T-P9-002 | Update delegation-enforcer.cjs | `.claude/hooks/delegation-enforcer.cjs` | qwen | [ ] |
| T-P9-003 | Update auto-project-create.cjs | `.claude/hooks/auto-project-create.cjs` | qwen | [ ] |
| T-P9-004 | Update settings.json with all hooks | `.claude/settings.json` | qwen | [ ] |
| T-P9-005 | Integration test all hooks | N/A | Claude | [ ] |
...
### Goal
Systems for detecting and fixing decomposition issues.
