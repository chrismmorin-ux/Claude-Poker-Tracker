...
| ID | Name | Code | State |
|----|------|------|-------|
| homebase-view | Homebase (default entry + dashboard + nav hub) | `src/components/views/HomebaseView/` | ● (2026-06-19; dashboard + throughpoint 2026-06-20, plan shimmying-moseying-lantern) |
...
| calibration-dashboard | Calibration Dashboard | `src/components/views/CalibrationDashboardView/` (Phase 5/6) | ● (EAL Gate 4 S4 + EDEV Phase 6 — cross-project spec; not yet implemented — registered in `viewRegistry` as `deferred: true` so `SCREEN.CALIBRATION_DASHBOARD` renders an explicit stub, not nothing, until built) |
...
- 2026-06-19 — Added `homebase-view` (default app-entry launchpad; replaced TableView as the cold-start default). Plan shimmying-moseying-lantern.
- 2026-06-20 — Homebase expanded to dashboard + navigation throughpoint after a full `design-audit` (E3·O1·B3·A1; the audit serves as this work's Gate 2 Blind-Spot Roundtable, evidence in `.claude/workstream/evidence/design/`). Shipped: view-registry refactor (`src/constants/viewRegistry.jsx` — single source of truth; closed the `CALIBRATION_DASHBOARD` silent-render drift), app-root `HealthIndicator` (operator surface L1→L2), Homebase results dashboard (reuses `InsightsBand`/`BankrollChart`) + "since your last visit" band for the Lapsed Returner persona, and `NavShell` one-tap Home throughpoint. New cross-surface artifact `navigation-ia.md`. See `homebase-view.md` change log.
