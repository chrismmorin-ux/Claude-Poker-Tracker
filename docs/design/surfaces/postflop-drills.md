...
├──────────────────────────────────────────────────────────────┤
│ Tab bar:                                                     │
│   [Pre-Session] [Line] [Range Explorer] [Range Estimate]     │
│   [Board Frameworks] [Scenario Library] [Lessons]            │
├──────────────────────────────────────────────────────────────┤
│ Mode content:                                                │
│                                                              │
...
- 2026-07-31 — **Tab label collision fix** (entry audit `2026-07-31-entry-drill-tab-label-collision.md`, GREEN). `Estimate Drill` → **Range Estimate**, `Framework Drill` → **Board Frameworks**, `Library` → **Scenario Library** — each previously collided verbatim with a `PreflopDrillsView` tab operating on a *different object* (range composition vs matchup equity; `board_tilt`/`whiff_rate` lenses vs `domination`/`race` lenses; curated scenarios vs curated matchups). Extends the rule WS-231 F-DRILL-03 established for `Explorer` → `Range Explorer`: **a label names its object; the street comes from the view.** `Lessons` deliberately still collides — same object, street differs. Internal tab `id`s unchanged. `LibraryMode`'s in-panel heading updated to match. Guarded by `views/__tests__/drillTabLabels.test.js`.
