...
**Status:** DECIDED 2026-08-01 (founder: *"appwide"*) · Item `WS-319` (id lost to the 2026-08-07 collision) / SPR-162
...
**Consumed by:** `WS-441` (touch-floor enforcement) and `WS-489` (layout migration). Original consumers `WS-320` / `WS-313` / `WS-322` were lost to the id collision.

> **ID-collision notice (2026-08-15, WS-441 recovery):** every `WS-3xx` id in this
> document predates the 2026-08-07 ticket-ID collision that overwrote the TVR queue
> entries with unrelated tickets. Where an id appears below it is annotated with its
> live successor. Live ids: **WS-441** (44px touch floor + enforcement, re-file of
> WS-316) and **WS-489** (the layout migration, re-file of the lost WS-322).
...
**Overflow.** Street tabs pin to the top, control zone pins to the bottom (both `flex-shrink: 0`); only the middle band scrolls. The two anchors `glance-return-chris` depends on never move, and **Next Hand becomes structurally unclippable — the CommandStrip-compression defect (original `WS-311`, id lost) closes via `WS-489`**, not separately.
...
- **This is a real migration, not a tweak.** ~9 views still scale. Re-filed as `WS-489` (original `WS-322` id lost), sequenced behind the doctrine and aligned with the June audit's Phase 3.
...
- **The orientation gate (original `WS-315`, id lost) is GONE as of WS-440 (2026-08-13):** the canvas now auto-rotates in portrait instead of gating. The paragraph below is preserved for the record; its premise no longer holds.
  **The orientation gate changes character.** Once most views are fluid, "please rotate to landscape" applies to a shrinking set of surfaces. It does not fix `WS-315` — the founder's report is device-specific and still needs reproduction — but it shrinks the blast radius.
...
**Recommendation: interactive/fluid.** Showdown is entered *after* the hand; the spatial map has done its job by then, and the task is data entry. But it mirrors the table's seat layout, and breaking that correspondence has a recognition cost worth naming before it is decided. Deferred to `WS-489` Phase D.
