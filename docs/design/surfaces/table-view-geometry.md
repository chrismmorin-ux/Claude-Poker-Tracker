...
**Item:** `WS-319` (id lost to the 2026-08-07 collision) · **Sprint:** SPR-162 · **Status:** ✅ DECIDED 2026-08-01
...
**Consumed by:** `WS-441` (touch-floor enforcement) and `WS-489` (layout migration). Original consumers `WS-320` / `WS-313` were lost to the id collision.
...
That kills the CommandStrip-compression ticket's proposed fix (c) (original `WS-311`, id lost; now `WS-489`) — *"collapse the spacer under pressure before any control shrinks"* — outright. The spacer has already collapsed; the compression I measured is what happens *after* it ran out.
...
### The CommandStrip-compression defect (original WS-311) closes here

Under (b) plus the scroll region, the Next Hand CTA is pinned and cannot be clipped. **it should be closed by `WS-489`, not fixed separately** — fixing it first would be work done twice.
...
See [`layout-doctrine.md`](./layout-doctrine.md). Migration re-filed as `WS-489` (original `WS-322` id lost).
