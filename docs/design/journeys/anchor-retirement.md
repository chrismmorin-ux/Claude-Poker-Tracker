...
### Variation — Global library reset (red line #4b; WS-221 / SPR-126)

The library-wide counterpart of per-anchor Reset — one owner action resets calibration across **every** anchor at once. **Entry point is Settings**, not the anchor surface: the "Reset calibration" danger-zone card in the Anchor Calibration area (`surfaces/settings-view.md` §EAL-G4-RESET), since global "reset all" controls conventionally live in Settings.

Step 2 modal copy: **"Reset all anchor calibration?"** + sub-text: *"Resetting drops accumulated calibration for all N anchors and restarts Tier 2 from seed priors. Evidence history is preserved but no longer contributes to the posteriors. This action cannot be auto-undone after the 12s toast window."* (generated by `buildLibraryResetCopy(anchorCount)`).

**Same confirm sheet, same discipline.** Reuses the shared `RetirementConfirmModal` destructive 2-tap path (checkbox "I understand all anchor calibration will be reset" gates `[ Reset all ]`) + 12s undo toast — single source of truth, no journey drift. Orchestrated by `useLibraryReset` (parallels `useAnchorRetirement`).

**Scope (founder decision):** anchors only — dispatches `LIBRARY_CALIBRATION_RESET`, which stamps `operator.calibrationResetAt` + override provenance on every anchor; **status unchanged, `anchorObservations` preserved** (red line #3 durability). Writer: W-EA-5. The undo path restores the exact prior anchors snapshot (`restoreAnchors`). The button is disabled when the library has 0 anchors.
...
- **C2 — Global library reset** (destructive; 2-tap confirm; entered from Settings; resets all anchors at once — red line #4b).
...
- 2026-06-13 — Added Variation C2 (Global library reset, red line #4b; WS-221 / SPR-126). Library-wide counterpart of per-anchor Reset, entered from Settings (`surfaces/settings-view.md` §EAL-G4-RESET). Reuses the shared `RetirementConfirmModal` 2-tap confirm + 12s undo via `useLibraryReset`; `LIBRARY_CALIBRATION_RESET` reducer action (W-EA-5). Scope: anchors only — stamps `calibrationResetAt` on every anchor; observations preserved.
