...
| 6 | `co-vs-bb-srp-oop-paired-k77` | CO / BB | 5.5bb | K♠7♦7♣ | not pinned — prose names hero "has all Kx and overpairs" in aggregate | `evidence/lsw-h3-06-k77-flop_root.png` | n/a | **B1 target:** paired board small-cbet. Natural pin: `A♥K♦` (TPTK on paired board). `bucketCandidates: ['topPair']`. Paired-board `trips` bucket is feasible for the range (A7s/K7s) but not for the AK pin — exclude `trips`. Overpair (AA/QQ-88) an alt-pin teaching variant — `bucketCandidates: ['overpair']`. **Validator note:** G5.1 taxonomy now splits topPair into good/weak; AKo on K77 would classify `topPairGood` — either alias or switch `bucketCandidates` to `['topPairGood']` (consistency check B1 should resolve before authoring). |
...
| `co-vs-bb-srp-oop-paired-k77` | yes | `['A♥K♦']` | `['topPair']` or `['topPairGood']` (taxonomy call) | no |
