...
**Status:** APPROVED by owner 2026-04-12 (all 5 sign-off decisions approved). **Second amendment round 2026-04-12** — UX-model reframe (R-1.2/R-1.5/R-1.3). **Third amendment round 2026-04-27** — Cross-zone consistency invariants (R-1.6/R-1.7). **Fourth amendment round 2026-04-28** — Mechanism coherence (R-1.8 + INV-FRESH-1..5). **Fifth 2026-04-28** — Token-semantic isolation (R-1.9 + INV-TOKEN-1..5). **Sixth 2026-04-28** — Affordance vocabulary discipline (R-1.5 amendment + R-1.10 + INV-AFFORD-1..5). **Seventh 2026-04-28** — Status concept-class discipline (R-1.11 + INV-STATUS-1..5). **Eighth amendment round approved 2026-04-28 (fifth edit same day)** — Density rhythm + attention-budget discipline: R-1.12 added (typography ladder + spacing tokenization + F-6 lexical exclusivity + minimum tap-target + attention-budget declaration + INV-DENSITY-1..5 behavioral invariants). **Closes shell-spec §I-§VI — full design-language doctrine in place.** See §11 amendment log.
**Date:** 2026-04-28
**Doctrine version:** 8 (v1 = initial approval; v2 = UX-model reframe; v3 = cross-zone consistency invariants; v4 = mechanism coherence; v5 = token-semantic isolation; v6 = affordance vocabulary discipline; v7 = status concept-class discipline; v8 = density rhythm + attention-budget discipline, 2026-04-28)
...
- **R-1.12** Every typography size, spacing value, sample-count lexical form, interactive tap-target, and zone attention-tier in the sidebar is declared in the closed-vocabulary §VI density registry in `docs/design/surfaces/sidebar-shell-spec.md`. Behavioral invariants INV-DENSITY-1..5 are binding.
  - **INV-DENSITY-1 (Typography unit discipline).** Every `font-size` declaration on user-readable content MUST use `rem` units. `px` font-size declarations outside the closed exception list (decorative-glyph slots; icon-only contexts) are spec violations. Lint-enforced via `density-registry.test.js` grep test (pattern: `font-size:\s*\d+px` outside design-tokens.js + exception list). **WCAG SC 1.4.4 (Resize Text, AA) cite.** **Grandfathered allowlist** for current 5 violations (`9px` stale-badge at `side-panel.html:74`, `10px` invariant-badge at `:84`, plus 3 others); explicit Gate 5 milestone for full compliance. Closes FM-DENSITY-1 (9px text illegible on Galaxy A22 DPR — currently shipping) + FM-DENSITY-2 (px tokens block OS-level zoom).
  - **INV-DENSITY-2 (Typography token completeness + spacing tokenization).** Every typographic role in the sidebar has exactly one declared `--type-*` token covering font-size + font-weight + color-tier mapping. Every spacing value (padding / margin / gap) consumes either a `--sp-*` raw scale token or a zone-tier semantic token (`--zone-{chrome,advice,content,deep}-padding`). Inline `padding: <px>` / `margin: <px>` / `gap: <px>` literals outside the grandfathered allowlist are spec violations. Lint-enforced. **Grandfathered allowlist** for current ~89 inline spacing-literal sites in `side-panel.html`; Gate 5 milestone for migration. Closes the 132 font-* / 89 spacing-literal drift surface that V-color-tokens did not reach (color tokens were locked; spacing/typography were left underspecified).
  - **INV-DENSITY-3 (F-6 lexical exclusivity).** The shipped form for sample-count rendering is **two intentional forms**, mutually exclusive by context:
    - `n=N` form (e.g., `n=45`) — paired with confidence dot per §III.4 (engine-model training-set sample size). Used at Z2 unified header + Z4 glance tier.
    - `Nh` form (e.g., `45h`) — standalone, no dot pairing (cumulative observation count from `cachedSeatStats[*].sampleSize`). Used at Z3 stat chips.
    No mixing within a single render context. Lint-enforced via `density-registry.test.js` fixture-output scan. Closes FM-DENSITY-3 (no test asserts absence of wrong form in render output today; future contributors reading documentation alone could re-introduce mixed corpus undetected).
  - **INV-DENSITY-4 (Minimum tap-target declaration).** Every interactive element declares its tap-target size in its Stage 4 spec. **WCAG 2.5.5 ≥44×44 CSS-pixel floor.** Below-floor elements require an explicit grandfathered-allowlist entry in `affordance-registry.test.js` (extends INV-AFFORD-4 allowlist with tap-target-size column) with documented rationale + Gate 5 milestone. Seat-arc circles (32×32px with ~20px center-to-center spacing on arc-end seats per FM-DENSITY-4 — silent wrong-villain-pin risk under `:hover scale(1.15)` overlap) are the canonical existing violation; grandfathered with explicit geometry note.
  - **INV-DENSITY-5 (Attention-budget declaration).** Every zone declares its attention-tier per system-state in §VI's attention-budget map. Closed enumeration of system states: `active-hand-advice` / `active-hand-analyzing` / `between-hands` / `between-hands-tournament` / `fatal-error` / `empty-no-table`. Closed enumeration of tiers: `PRIMARY` / `supporting` / `secondary` / `ambient` / `available-collapsed` / `suppressed`. New zones, new system states, or new tier values require doctrine amendment via §11. Closes the Gate 1 §4.2 missing-artifact "Attention budget — documented hierarchy of how zones compete for eye time."
  - *Closed enumeration of typography ladder:* exactly 3 tiers + 1 modifier class. `--type-display` (1.5rem / 24px-equivalent — action badge label + EV number + fold-% number); `--type-body` (0.875rem / 14px — pot size + SPR + equity + board cards + recommendation text); `--type-meta-stat` (0.6875rem / 11px — sample count + confidence label + action-amount under seat arc). Plus `.tabular-nums` CSS class modifier for column-aligned numerics (binds `font-variant-numeric: tabular-nums` — CSS variables can't natively bind this property). New tiers require doctrine amendment.
  - *Closed enumeration of spacing tokens:* `--sp-1..6` (4/8/12/16/20/24px raw scale, retained as-is) + zone-tier semantic tokens (`--zone-chrome-padding`, `--zone-advice-padding`, `--zone-content-padding`, `--zone-deep-padding`). New token-tier additions require doctrine amendment.
  - *Density toggle (deferred to V-density v2):* per GTO Wizard precedent (LEDGER `EVID-2026-04-27-COMP-GTOWIZARD` "product-level density ladder is a first-class user setting"), a future user-controlled comfortable/compact toggle is anticipated. v8 lands token groundwork (`--density-zone-gap`, `--density-row-min`); UI toggle deferred.
  - *Prevents:* the 132 inline `font-*` declarations + 89 inline spacing-literal violations cataloged by V-density 5-specialist roundtable; F-6 lexical re-entry via documentation-vs-code drift; tap-target crowding at 9-seat arc geometry; cross-product typography divergence (currently undetectable since INV-TOKEN-4 mirror-lock checks color hex only); WCAG SC 1.4.4 violation (9px font ships illegible on Galaxy A22 DPR today).
  - *Constrains:* Stage 4 specs that introduce text content, spacing, interactive elements, or zone-level attention claims must declare entries in the §VI registry. Stage 6 PR gate verifies (a) `font-size` references a `--type-*` token; (b) spacing references `--sp-*` or `--zone-*-padding` token; (c) tap-target ≥44×44 OR allowlist entry; (d) lexical form matches §III.4 pairing rules; (e) attention-budget map updated for new zones / states.
  - *Companion test infrastructure:* `density-registry.test.js` (parallel to `freshness-signal-registry.test.js`, `design-token-registry.test.js`, `affordance-registry.test.js`, `status-registry.test.js`) parses §VI density registry; asserts INV-DENSITY-1..5. `dom-mutation-discipline.test.js` extension covers tap-target floor. Test infrastructure ships with the rule, not "later."
  - *Does NOT prescribe:* the specific `rem` value at each tier (per-element-spec scope per §10); the exact pixel values of `--sp-*` (also §10); the typography weight values; per-element padding/margin within the tier's range. R-1.12 governs the *taxonomy + behavioral invariants*, not per-element pixel choice — same structural-vs-visual line as R-1.6/1.7/1.8/1.9/1.10/1.11.
  - *Forensics origin:* surfaced by SHC Gate 3 architectural pass (Cross-cutting Findings F-6 lexical inconsistency + F-7 decorative-glyph drift — both deferred to §VI density at the time) and ratified by SHC V-density 5-specialist roundtable (2026-04-28). systems-architect specialist identified the 6-name-→-3-size token-ladder bug at `design-tokens.js:104-115` + the 24px action-word-hardcoded-outside-tokens bug; cto-agent specialist counted 132 inline `font-*` + 89 spacing-literal declarations across HTML + JS + reversed their V-status "no new rule" position on the grounds that V-density's evidence is the strongest yet for binding-doctrine + lint-enforcement; senior-engineer specialist surfaced the test-suite asymmetry argument (4 doctrine rules with lint tests, R-1.12 without would create survivor-bias drift); failure-engineer specialist contributed FM-DENSITY-1..6 (including FM-DENSITY-1 9px-illegible-on-Galaxy-A22-DPR currently shipping, plus FM-DENSITY-4 seat-arc tap-target crowding) + identified WCAG SC 1.4.4 + SC 2.5.5 compliance violations + cautioned against `Object.freeze` on typography tokens until rem migration completes; product-ux-engineer specialist authored the 6-row attention-budget map (state × zone × tier) using all 5 harness fixtures + framed the typography ladder as ratio-based (not absolute px) for compatibility with the existing scale system.
  - *Added (2026-04-28, owner-approved, v8). Closes the shell-spec §I-§VI design-language doctrine.*
...
**v8 — 2026-04-28 (density rhythm + attention-budget discipline — closes §I-§VI shell-spec sequence)**

Source: SHC V-density 5-specialist roundtable (2026-04-28). §VI density-rhythm + attention-budget was the last unresolved shell-spec section after V-2/V-3/V-color-tokens/V-affordance/V-status closed §I-§V. v8 closes §VI with the binding-doctrine + lint-test pattern established by v4-v7. Strengthened long-term lens (per `memory/feedback_long_term_over_transition.md`) operationalized for the **fifth consecutive doctrine amendment**.

**Forensics expansion (FM-DENSITY-1..6 + token-ladder bug):**
- **`design-tokens.js:104-115` 6-name → 3-size token ladder bug.** `font-micro/xs/sm/base` all alias `11px`; `font-md/lg` both `14px`; `24px` action-word HARDCODED outside any token at `side-panel.html:400`. Names lie about relative scale — typography equivalent of the `#fbbf24` × 7 token-roles problem that R-1.9 was written to prevent.
- **132 inline `font-*` declarations + 89 inline spacing-literal violations** across `side-panel.html` + render-* JS template literals (cto-agent codebase forensics).
- **FM-DENSITY-1 (9px stale-badge font illegible on Galaxy A22 DPR — currently shipping).** `side-panel.html:74` declares `font-size: 9px`. At Samsung Galaxy A22 device-pixel-ratio, effective rendered size is sub-6px physical pixels. WCAG 2.1 SC 1.4.4 violation. User reading "Stale 23s" cannot distinguish text. Currently in production.
- **FM-DENSITY-2 (px tokens block OS-level zoom).** All `--font-*` tokens are px strings; CSS cascade respects px unit, not root em. User with OS accessibility zoom (150%) sees no scaling. Silent.
- **FM-DENSITY-3 (F-6 re-entry path open).** No test asserts absence of `n=N` form in render output where `Nh` was intended. INV-TOKEN-1 only checks hex patterns. Future contributors reading documentation alone can re-introduce mixed corpus undetected.
- **FM-DENSITY-4 (Tap-target crowding at 9-seat arc).** Seat circles 32×32px at `side-panel.html:131`; arc-end seats overlap (~20px center-to-center) under `:hover scale(1.15)`. Below WCAG 2.5.5 44px minimum. Wrong-villain-pin failure mode silent.
- **FM-DENSITY-5 (Object.freeze speculation).** V-color-tokens precedent froze color tokens. If applied to typography NOW, pre-rem-migration, future rem migration becomes breaking API change. Avoid.
- **FM-DENSITY-6 (Cross-product typography divergence undetectable).** INV-TOKEN-4 mirror-lock checks color hex; typography keys aren't shared. Test passes silently while products diverge.

Rule added:
- **R-1.12 (new).** Density-rhythm + attention-budget discipline + INV-DENSITY-1..5 behavioral invariants. Closed enumerations: 3-tier typography ladder + `.tabular-nums` modifier class; `--sp-*` raw spacing + `--zone-*-padding` semantic tokens; F-6 two-form lexical exclusivity (`n=N` paired with confidence-dot vs `Nh` standalone); WCAG 2.5.5 ≥44×44 tap-target floor; closed system-state × zone-tier attention-budget map.

Companion infrastructure committed by R-1.12:
- **`density-registry.test.js`** — parses §VI density registry; asserts INV-DENSITY-1..5 (rem unit grep; `--type-*` / `--sp-*` token-completeness scan; F-6 lexical exclusivity fixture-output scan; attention-budget axis enumeration check).
- **`dom-mutation-discipline.test.js`** — extended for tap-target ≥44×44 floor (INV-DENSITY-4) + lexical-form scan (INV-DENSITY-3).
- **`affordance-registry.test.js`** — extended with tap-target-size column on its existing INV-AFFORD-4 grandfathered allowlist (per V-affordance Class B exception list pattern).
- **`shared/design-tokens.js` migration** — `--type-display` / `--type-body` / `--type-meta-stat` (rem-based) + `--zone-{chrome,advice,content,deep}-padding` semantic spacing tokens. Deprecation alias mechanism: `injectTokens()` emits both old `font-*` and new `type-*` during transition (parallel to V-color-tokens deprecation alias mechanism). NO `Object.freeze` on typography/spacing tokens until rem migration completes (FM-DENSITY-5 caution).
- **`.tabular-nums` CSS class modifier** authored — applied alongside any `--type-*` tier; binds `font-variant-numeric: tabular-nums` for column-aligned numerics (CSS variables can't bind this property natively).

§10 not amended under v8. R-1.12 governs **token taxonomy + behavioral invariants** (closed-vocabulary discipline parallel to R-1.6-R-1.11), not per-element pixel choice (which remains §10 scope per the structural-vs-visual line preserved across v3-v8).

Long-term lens: same canonical pattern as v4-v7. cto-agent specialist's V-status "no new rule" position **reversed at v8** on the grounds that V-density's evidence (132 + 89 violations + currently-shipping WCAG 1.4.4 failure + 6 silent FM-DENSITY entries) is the strongest yet for binding-doctrine + lint-enforcement.

Rules added under v8 apply to all Stage 4 specs authored hereafter. Pre-existing zone surface artifacts NOT retroactively forced; remediation per Gate 5 cadence with grandfathered allowlists for the 5 inline-px violations + ~89 spacing-literal sites + tap-target floor exceptions.

**v8 required co-shipping (12 items):** `--type-display` / `--type-body` / `--type-meta-stat` token entries (rem-based) + `--zone-*-padding` semantic spacing tokens; deprecation alias mechanism in `injectTokens()`; `.tabular-nums` CSS class modifier; 5 inline `font-size: <px>` literals migrated or grandfathered (`9px` stale-badge, `10px` invariant-badge, `11px` seat-sample, `14px` body, `24px` action-word); ~89 inline spacing literals migrated or grandfathered allowlist; `density-registry.test.js`; `dom-mutation-discipline.test.js` tap-target + lexical-form extensions; `affordance-registry.test.js` tap-target-size column extension; `STATUS_AXES`-style attention-budget meta-file entries; mirror-lock test scope decision (extension-only for v1; main-app reconciliation deferred to V-5 per Tailwind boundary). NO `Object.freeze` on typography/spacing tokens until rem migration completes.

**v8 cross-cutting amendments:**
- §V (V-color-tokens) extended with `--type-*` typography tokens (3 entries) + `--zone-*-padding` zone-tier spacing tokens (4 entries) + `.tabular-nums` modifier-class binding.
- §III (V-2 confidence) typographic-ladder reference updated to cite §VI tier names (`--type-meta-stat` for confidence labels and sample-count text per §III.4).
- §I-§V boundary clarifications: §VI attention-budget map uses tier vocabulary distinct from §3 R-3.* interruption-tier vocabulary (per failure-engineer's boundary insight — §3 governs *preemption rights*; §VI governs *visual-weight allocation*).

**v8 closes the shell-spec §I-§VI sequence.** Doctrine v8 = 8 binding rules in §1 (R-1.1 through R-1.12) + 5 categories of behavioral invariants (INV-FRESH/TOKEN/AFFORD/STATUS/DENSITY-1..5) + 5 companion test infrastructures (`freshness-signal-registry.test.js`, `design-token-registry.test.js`, `affordance-registry.test.js`, `status-registry.test.js`, `density-registry.test.js`) + extended `dom-mutation-discipline.test.js`. **Gate 4 of SHC project effectively closes with v8 amendment.**
